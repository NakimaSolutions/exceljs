...
class WorksheetWriter {
  ...
  _writeMedia() {
    this._media.forEach(medium => {
      if (medium.type === 'background') {
        const image = this._workbook.getImage(medium.imageId);
        ...
        this.stream.write(xform.picture.toXml({rId: this.background.rId}));
        ...
        if (!this.drawing) {
          ...
          this._write(`<drawing r:id="${drawingId}" />`);
          this.workbook.drawings.push(this.drawing);
        }
        const image = this._workbook.getImage(medium.imageId);

        const rIdImage = this._sheetRelsWriter.addMedia({
          Target: `../media/${image.name}`,
          Type: RelType.Image,
        });
        this.drawing.rels.push({
          Id: rIdImage,
          Type: RelType.Image,
          Target: `../media/${image.name}`,
        });
